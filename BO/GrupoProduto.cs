     ...
     public class GrupoProduto
    {
        ...
        private void Load()
        {
            try
            {
                ...
                {
                    dr.Read();
                    try { this._GRP_CODIGO = dr.GetSqlInt32(0).Value; }  catch { this._GRP_CODIGO = 0; }
                    try {  this._GRP_NOME   = dr.GetSqlString(1).Value;} catch { this._GRP_NOME   = "";}
                    try { this._GRP_OBS = dr.GetSqlString(2).Value; }   catch { this._GRP_OBS = ""; }
                }
                else
                    this._GRP_CODIGO = 0;
            }
