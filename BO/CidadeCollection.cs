    ...
    public class CidadeCollection : List<Cidade>
     {
        ...
        public CidadeCollection(int CID_ID, int ORDER)
        {
        ...
        public CidadeCollection(string CID_CODIGO, int ORDER)
        {
            this._typeLoad = CidadeLoadType.LoadByCidadeCodigo;
            ...
            try
            {
                switch (this._typeLoad)
                {
                    case CidadeLoadType.LoadAll:
                        this.cmd = new SqlCommand("SELECT CID_ID,CID_CODIGO, CID_NOME, CID_UF FROM CIDADE " + Order(), this.con);
                        cmd.CommandType = CommandType.Text;
                        break;
                    case CidadeLoadType.LoadById:
                        this.cmd = new SqlCommand("SELECT CID_ID,CID_CODIGO, CID_NOME, CID_UF FROM CIDADE WHERE CID_CODIGO = @CID_CODIGO " + Order(), this.con);
                        this.cmd.CommandType = CommandType.Text;
                        this.cmd.Parameters.Add("@CID_ID", SqlDbType.Int);
                        this.cmd.Parameters[0].Value = this._CID_ID;
                        break;
                    case CidadeLoadType.LoadByCidadeCodigo:
                        this.cmd = new SqlCommand("SELECT CID_ID,CID_CODIGO, CID_NOME, CID_UF FROM CIDADE WHERE CID_CODIGO = @CID_CODIGO " + Order(), this.con);
                        this.cmd.CommandType = CommandType.Text;
                        this.cmd.Parameters.Add("@CID_CODIGO", SqlDbType.VarChar);
                        this.cmd.Parameters[0].Value = this._CID_CODIGO;
                        break;
                    case CidadeLoadType.LoadByCidadeUF:
                        this.cmd = new SqlCommand("SELECT CID_ID,CID_CODIGO, CID_NOME, CID_UF FROM CIDADE WHERE CID_UF = @CID_UF " + Order(), this.con);
                        cmd.CommandType = CommandType.Text;
                        cmd.Parameters.Add("@CID_UF", SqlDbType.VarChar);
                        cmd.Parameters[0].Value = this._CID_UF;
                        break;
                    case CidadeLoadType.LoadByCidadeNome:
                        this.cmd = new SqlCommand("SELECT CID_ID,CID_CODIGO, CID_NOME, CID_UF FROM CIDADE WHERE CID_NOME COLLATE Latin1_General_CI_AI LIKE '%' + @CID_NOME + '%' " + Order(), this.con);
                        this.cmd.CommandType = CommandType.Text;
